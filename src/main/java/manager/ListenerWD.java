...
public class ListenerWD extends AbstractWebDriverEventListener {
    ...
    public void onException(Throwable throwable, WebDriver driver) {
        ...
        logger.info("Huston we have problems");
        logger.info(throwable.getMessage());
        logger.info(throwable.fillInStackTrace().toString());
        int i = (int)(System.currentTimeMillis()/1000%3600);
        String link = "src/test/screenshots/screen-"+i+".png";
        HelperContact helperContact = new HelperContact(driver);
        helperContact.getScreen(link);
        logger.info("This link is to screenshot with error --> "+link);
    }
